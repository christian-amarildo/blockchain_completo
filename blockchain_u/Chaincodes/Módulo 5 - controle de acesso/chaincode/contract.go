...
////////////////  MÓDULO 5 ////////////////
//Função que realiza controle de acesso. Emite erro se o chamador não tiver o valor "admin" na OU, em seu ID.
func checkIfAdmin(ctx contractapi.TransactionContextInterface) error {
	//Pega o ID do chamador (em base64)
	clientID, err := ctx.GetClientIdentity().GetID()
	if err != nil {
		return fmt.Errorf("Falha ao obter o ID do usuário: %v", err)
	}

	//Decodifica o ID de base64 para texto normal 
	decodedClientID, err := base64.StdEncoding.DecodeString(clientID)
	if err != nil {
		return fmt.Errorf("Erro ao decodificar o ID: %v", err)
	}

	//Pega o "role" (campo OU) do identificador, usando regex
	re := regexp.MustCompile("^x509::CN=.*?,OU=(admin|client).*$")
	match := re.FindStringSubmatch(string(decodedClientID))

	//Emite erro se o "role"(OU) do chamador não dor "admin"
	if match[1] != "admin" {
...
// Observação: esta função se chamaca originalmente ClientAccountBalance (nome pouco intuitivo)
...
func authorizationHelper(ctx contractapi.TransactionContextInterface) error {
	...
	//Pega a org do chamador e coloca em result
	result, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return fmt.Errorf("failed to get MSPID: %v", err)
	}

	if result != (authorizedOrg+"MSP") {
		return fmt.Errorf("Organização não autorizada!")
	}
...
func mintHelper(ctx contractapi.TransactionContextInterface, operator string, account string, id string, amount uint64) error {
	...
	if amount <= 0 {
		return fmt.Errorf("Quantidade emitida deve ser um inteiro positivo")
	}
